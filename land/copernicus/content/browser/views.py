...
class MultiDownloadView(BrowserView):
    ...
    def values(self):
        ...
        selected_landfiles = filter(None, self.request.form.get(
            'selected', None).split('@'))
        selected_landfiles_brains = [
...
class RedirectDownloadUrl(BrowserView):
    ...
    def url_login_with_params(self):
        ...
        land_item_url = self.context.absolute_url()
...
class OldRedirectDownloadUrl(BrowserView):
    """ Redirect to download url for a LandItem if logged in
        [TODO] Remove this.
    """
